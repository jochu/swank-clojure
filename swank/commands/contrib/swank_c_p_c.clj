...
(defn- compound-prefix-match?
  "Takes a `prefix' and a `target' string and returns whether `prefix'
   ...
   split by `split', if each substring of `prefix' is a prefix
...
(defn- delimited-compound-prefix-match?
  ...
  ([delimiter prefix target]
     (compound-prefix-match? #(.split % (str "[" (java.util.regex.Pattern/quote delimiter) "]") -1)
                             prefix
                             target)))
...
(defn- split-compound-prefix-match? [symbol-string potential]
...
(defn- compound-prefix-match-acronyms?
  ...
  ([delimiter prefix target]
     (or (delimited-compound-prefix-match? delimiter prefix target)
         (delimited-compound-prefix-match? delimiter (unacronym delimiter prefix) target))))
